...
class QualifiedExpressionResolver {
    ...
    fun processImportReference(
        ...
        val packageFragmentForCheck =
            importDirective.containingFile?.let { containingFile ->
                when {
                    containingFile.suppressDiagnosticsInDebugMode() ->
                        null
    ...
    private fun KtImportLike.ImportContent.asQualifierPartList(): List<QualifierPart> =
        when (this) {
            is KtImportLike.ImportContent.ExpressionBased -> expression.asQualifierPartList()
            is KtImportLike.ImportContent.FqNameBased -> fqName.pathSegments().map { QualifierPart(it) }
        }
